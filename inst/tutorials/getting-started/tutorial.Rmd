...
The red rectangle highlights that we are now running two Terminals, which we can switch back-and-forth between. In fact, we can have multiple Terminals and multiple Consoles in one Positron window just by using the `+` button. Instead of using a second Positron window to have an open Console for use while working on a tutorial, some people prefer this approach.

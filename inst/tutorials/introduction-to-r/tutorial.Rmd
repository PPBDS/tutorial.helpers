...
In the Console, hit the Up Arrow to retrieve the previous command. Edit it to add the argument `n = 4` to `slice_sample(diamonds)`. That is, you want `slice_sample(diamonds, n = 4)`. This will return 4 random rows from the `diamonds` data set.
...
Instead of using the Console, we will be doing the exercises in this section using exercise blocks.
...
<!-- HK: Exercises 10, 11, and 12 hit the time limit error when running the full pipe chain. Consider breaking into smaller steps or increasing the time limit -->

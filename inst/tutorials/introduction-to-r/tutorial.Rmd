...
Although the *Tidyverse* includes hundreds of commands for data manipulation, the most important are `filter()`, `select()`, `arrange()`, `mutate()`, and `summarize()`. Whenever you face a new problem, try to think about which one of these commands might be a good way to start.
...
The number of rows has decreased because we removed rows with missing values. `drop_na()` remove all rows which have a missing value for any of the variables. If we wanted yo just remove the rows which have are missing `tvhours`, we would use `drop_na(tvhours)`.
...
Add another layer with `geom_point()` using the `+` sign. Plotting code in the **ggplot2** package uses `+`, not `->`, to connect different commands together. This difference comes from the fact that **ggplot2** was written 10+ years before the pipe was invented.

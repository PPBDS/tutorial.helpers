...
```{r copy-code-chunk, child = system.file("child_documents_es/copy_button.Rmd", package = "tutorial.helpers")}
```
...
```{r info-section, child = system.file("child_documents_es/info_section.Rmd", package = "tutorial.helpers")}
```
...
A menudo necesitaremos copiar código de nuestros ejercicios anteriores para continuar trabajando en un problema que se vaya a resolver por etapas. Para hacerlo más fácil, proporcionamos un botón que dice `Copy previous code` (Copiar el código anterior). Debajo del siguiente cuadro, haz clic en el botón `Copy previous code`.
...
```{r download-answers, child = system.file("child_documents_es/download_answers.Rmd", package = "tutorial.helpers")}
```

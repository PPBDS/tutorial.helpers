...
learnr::tutorial_options(
  ...
  # No gradethis checker:
  # exercise.checker = NULL (default)
...
if (in_ci) {
  # In CI, render code but never execute Python
  ...
  # Local/interactive bootstrap
  run_cmd <- function(cmd, args) {
    message(sprintf(">> %s %s", cmd, paste(args, collapse = " ")))
    status <- system2(cmd, args, stdout = TRUE, stderr = TRUE)
    invisible(status)
  }

  if (!nzchar(uv)) {
    message("`uv` not found; install from https://docs.astral.sh/uv/ or continue with reticulate fallback.")
  }

  # Create venv if missing
  if (!dir.exists(venv)) {
    if (nzchar(uv)) {
      try(run_cmd(uv, c("venv", "--python", "3.12", "--seed", venv)), silent = TRUE)
    } else {
      try(reticulate::virtualenv_create(envname = venv), silent = TRUE)
    }
  }

  # OS-appropriate python path
  py_bin <- if (is_windows) file.path(venv, "Scripts", "python.exe") else file.path(venv, "bin", "python")

  # Point reticulate at the venv if it exists
  if (file.exists(py_bin)) {
    Sys.setenv(RETICULATE_PYTHON = normalizePath(py_bin, winslash = "/", mustWork = FALSE))
  }

  # Install Python deps (local only)
  py_pkgs <- c("polars","pandas","matplotlib","seaborn","plotnine")
  if (nzchar(uv)) {
    try(run_cmd(uv, c("pip", "install", "--python", venv, "--upgrade", py_pkgs)), silent = TRUE)
  } else if (file.exists(py_bin)) {
    try(system2(py_bin, c("-m", "pip", "install", "--upgrade", py_pkgs)), silent = TRUE)
  }

  # Activate venv for reticulate and enable python engine
...
# Same pipeline as the plot, but stay in Polars (no pandas/plotnine)
tips_dinner = (
    tips
    .filter(pl.col("time") == "Dinner")
    .select(["total_bill", "tip", "sex", "day"])
...
# Show a small sample of the (x, y) pairs we would plot
print("Sample (total_bill, tip):")
print(tips_dinner.select(["total_bill", "tip"]).head(8))

# Simple numeric check of relationship to "validate" the would-be plot
xy = tips_dinner.select(["total_bill", "tip"]).to_numpy()
if xy.shape[0] >= 2:
    r = np.corrcoef(xy[:, 0], xy[:, 1])[0, 1]
    print(f"\nCorrelation(total_bill, tip) ≈ {r:.3f}")
else:
    print("\nNot enough rows to compute correlation.")
...
# Add tiny random noise ("jitter") and recompute correlation
rng = np.random.default_rng(0)
xy = tips_dinner.select(["total_bill","tip"]).to_numpy()
jxy = xy + rng.normal(scale=[0.25, 0.10], size=xy.shape)
np.corrcoef(jxy[:,0], jxy[:,1])[0,1]
...
if xy.shape[0] >= 2:
    ...
    print(f"Predicted tip for $50 bill ≈ {pred_50:.2f}")
else:
    print("Not enough rows to fit a line.")
```
...
Ask AI to create a beautiful plot using the `iris` dataset and the pandas/matplotlib/seaborn libraries. Run the provided code in the Console. If it fails, show the AI the error and ask for better code.

...
# --- Cross-platform .venv setup for this tutorial ---
# Creates a .venv in the tutorial folder (if not already there)
# and installs all required Python modules.
...
if (!nzchar(python_exe)) stop("No usable Python found on this system.")

# 2) Create the venv if it doesn’t exist yet
if (!dir.exists(venv)) {
  message("Creating .venv for this tutorial using Python at: ", python_exe)
  reticulate::virtualenv_create(envname = venv, python = python_exe)
}

# 3) Point reticulate to this environment
Sys.setenv(RETICULATE_PYTHON = normalizePath(py_bin, winslash = "/", mustWork = FALSE))
reticulate::use_virtualenv(venv, required = TRUE)

# 4) Upgrade pip and install required modules
pip_path <- if (is_windows) file.path(venv, "Scripts", "pip.exe") else file.path(venv, "bin", "pip")
system2(pip_path, c("install", "--upgrade", "pip", "setuptools", "wheel"), stdout = TRUE, stderr = TRUE)

reticulate::py_install(
  c("polars", "pandas", "numpy", "plotnine", "matplotlib", "seaborn"),
  envname = venv, pip = TRUE
)

# 5) Register reticulate's Python engine for learnr
knitr::knit_engines$set(python = reticulate::eng_python)

message("✅ .venv ready with all required Python packages.")
```

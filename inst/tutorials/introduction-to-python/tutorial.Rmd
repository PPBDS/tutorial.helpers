...
<!-- normalizePath(".venv/Scripts/python.exe", winslash = "/", mustWork = TRUE) fails on mac. Maybe only do on Windows? -->

<!-- Get rid of install if not there. Make like other tutorials. -->

<!-- Get rid of gradethis. Don't care if check shows up. -->
...
# Stop auto-probing Windows Store python
Sys.setenv(RETICULATE_AUTOCONFIGURE = "FALSE")
# Point directly at your venv
Sys.setenv(
  RETICULATE_PYTHON = normalizePath(".venv/Scripts/python.exe", winslash = "/", mustWork = TRUE)
)
```

```{r setup, include=FALSE}
# -------- R packages: auto-install --------
needed <- c("learnr","gradethis","tutorial.helpers","knitr","reticulate")
...
gradethis_setup(
  fail = gradethis::gradethis_demo_check_fail(),
  pass = gradethis::gradethis_demo_check_pass()
)
...
# -------- uv + venv bootstrap (NO py_config() before we set env) --------
venv <- ".venv"
uv <- Sys.which("uv")
...
# Create venv with a known-good Python (3.12) using uv
if (!nzchar(uv)) stop("uv not found on PATH. Install uv and retry.")
if (!dir.exists(venv)) {
  # Ensure uv has a 3.12 interpreter available; uv will fetch if needed
  run_cmd(uv, c("venv", "--python", "3.12", "--seed", venv))
}

# Set RETICULATE_PYTHON *before* any reticulate Python initialization
...
# Install Python deps inside the venv via uv pip (fast)
py_pkgs <- c("polars","pandas","matplotlib","seaborn","plotnine")
run_cmd(uv, c("pip", "install", "--python", venv, "--upgrade", py_pkgs))

# Now tell reticulate to use the venv (safe because env var already set)
reticulate::use_virtualenv(venv, required = TRUE)

# Enable Python engine for knitr

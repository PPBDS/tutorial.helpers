...
Feel free to copy/paste this question as-is, but replacing `XX` with whatever makes sense for your assignment. That is, you need to provide your own repo name, Quarto document title and so on. The repo name should be descriptive and also not likely to have conflicts with other repos in the students GitHUb account. So, `golf-scores` not `project-1`. Of course, the code chunk name might change when your run `tutorial.helpers::check_current_tutorial()`.

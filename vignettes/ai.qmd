...
<!-- AR: I feel like set up often occurs within topics in vs code tutorials. -->
...
<!-- AR: often, publish steps occur within topics and not in summary -->
...
<!-- AR: Does introductory prose go in between the topic title and the first "###" ? -->
...
The question will almost always instruct the student to "CP/CR," often after running `show_fle(chunk = "Last")`. Adjust the value of `rows` to match the expected length of the pasted output. If the question asks student to run some R code, we will generally include a reasonable answer in the `test` chunk. This serves two purposes. First, because `echo = TRUE`, we show the student that code (and its output) in the tutorial. This code should be excellent. We want students to learn from our example. Second, the code will be run when we test the tutorial, so we can be sure that this code (and, we hope, code like it) will work for the student.

<!-- AR: Don't we often show examples of what the answer should look like for these questions? Any guidance on this? Where does this go in relation to knowledge drops and the reponse input? -->
...
<!-- AR: By "two exercise code chunks" does this mean that there are two answer submissions within one exercise or two separate exercises? Are these the answer fields that can execute code? How do we set this up? -->
...
<!-- AR: Will the Arrow tutorial change? This may need to be updated in the future. -->
<!-- AR: How do we balance approach 1 (to have written exercises while running commands outside the tutorial) with approach 2 (code chunks in tutorial run on a smaller version of the data sets)  -->
...
<!-- AR: I find this section quite confusing! Is there any way to simplify? -->
...
<!-- AR: Should we specify more explicitly that this first chunk should have a "setup" label? -->
...
<!-- AR: where does a knowledge drop fit in in this type of question? after another "###"? None at all? -->
...
<!-- AR: There are lots of tutorials with more than 2 topics! -->
...
<!-- AR: do we prefer using R to something like curl in bash? Do both? -->
...
<!-- AR: Below, what is meant by "show students" ?-->
...
<!-- AR: How does the student know what they submit is correct for this exercise? -->

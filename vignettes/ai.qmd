...
Students always need more practice working in a Quarto document (the QMD) and the Console at the same time. Good data scientists go back and forth between these two modes, writing something in the QMD, executing it in the Console, editing the QMD, executing again, and so on. We need to force students to do that here.

AI tutorials begin with an Introduction which begins with a summary of the skills which the tutorial will cover. The Introduction continues with a series of exercises which set up the repo/project/QMD in which most of the tutorial will be completed.

After the Introduction, there are 3 to 5 Sections --- the official **learnr** nomenclature is "Topics" --- which are the meat of the tutorial. 

The last topic is the Summary. It starts with the same overview with which the Introduction began, but in the past tense. It then has a couple exercises which finish up the tutorial by using `quarto publish gh-pages analysis.qmd` to create a webpage with all the cool plots which the student has created. The URL for this new webpage is usually the answer to the last Exercise in Summary, thereby finishing the tutorial.
...
Feel free to use any of the example Exercises from this document. Soon, we will add options to `tutorial.helpers::make_exercises()` to make this easier. Either way, you will need to do some edits, like with `"XX.qmd"`. That is, any time you see `XX` in an example exercise, you need to replace it with something sensible. 
...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"XX"` -- and an author (you). Render the document and save it as `analysis.qmd`.
...
Feel free to copy/paste this question as-is, but replacing `XX` with whatever makes sense for your assignment. That is, you need to provide your own repo name, Quarto document title and so on. The repo name should be descriptive and also not likely to have conflicts with other repos in the students GitHub account. So, `golf-scores` not `project-1`. Of course, the code chunk name might change after you run `tutorial.helpers::check_current_tutorial()`.

You do not need to use `analysis.qmd` as the name of the QMD file which the student creates. But using the same name doesn't hurt anything and is convenient since it decreases the number of things which the user of these example exercises needs to change.

...
To create a new tutorial, you first need a new directory, located in the `inst/tutorials` directory of your package. Create that directory and then add a `tutorial.Rmd` file inside it. Tutorials must be named `tutorial.Rmd`.

Every tutorial begins with the `info_section`, some default code which records a student's name, email and (optionally) id. Include it near the top of the tutorial with a child document chunk:
...
Every tutorial ends with the `download-answers` chunk, which provides students with instructions on how to download a copy of their answers:

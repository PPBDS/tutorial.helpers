...
RStudio Addins are not available in Positron. To use these tools, run these commands from the Console.
...
`make_exercise()` is the R function to run, if you are using Positron.
...
`make_no_answer()` is the R function to run, if you are using Positron.
...
`make_yes_answer()` is the R function to run, if you are using Positron.

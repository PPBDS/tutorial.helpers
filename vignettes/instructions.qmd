...
<!-- Luke: Figure out if we can have line-wrapping within a verbatim environment. -->
...
The r chunk with `echo=TRUE` and `eval=TRUE` (which is default) allows us to see the code as well as see what it produces. This makes it easy for the students to copy and paste our code if they need to, and they can also look at the outputs to see if they match.

Using 4 backticks would only show the students whatever we type within the backticks. Additionally, it would not be an r chunk, so a person editing the tutorial would not easily be able to run any code within the backticks. On top of that, the 4 backticks are not able to evaluate any code within.

<!-- Luke: There are two ways for providing an answer to the question. One is in the r chunk, one is with four backticks. The former must be used if you are going to ask students to copy/paste your code into the QMD in place of their current code. -->

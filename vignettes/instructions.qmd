...
*Use our tools, especially functions like [`check_current_tutorial()`](https://ppbds.github.io/tutorial.helpers/reference/check_current_tutorial.html) and [`make_exercise()`](https://ppbds.github.io/tutorial.helpers/reference/exercise_creation.html).*
...
* Use high quality links. 
  ...
  * Anything associated with [Posit](https://posit.co/) is high quality. Examples include sites associated with [Tidyverse](https://tidyverse.org/) and [Tidymodels](https://www.tidymodels.org/).
...
Instructors have different needs and priorities when it comes to processing student answers. We provide the functions `submissions_summary()` and `submissions_answers()` to help them. 
...
1. Type `rmarkdown::render("file_path")` in the console. This will give you a file with the same file name and a `.html` extension. Right click the file and select `Open in Browser`. This open up a webpage and you can check the tutorial for some errors. It is smart to use `rmarkdown::render("file_path")` regularly since it will identify syntax errors quickly. A good practice is to render after completing each exercise. 

2. Do a full test, which means running `devtools::check()`. This is the equivalent of `R CMD check` which validates package structure, documentation, tests, and compliance with CRAN standards. A good result would be `0 errors ✔ | 0 warnings ✔ | 0 notes ✔`.

### What to do if `devtools::check()` fails

1. Read the error message carefully. You always want to see `0 errors ✔ | 0 warnings ✔ | 0 notes ✔` under the `R CMD check results`. If not, there is a problem. The error message will often provide a clue as to where in your code the error occurred.
